...
class Grupo extends Usuario {
    ...
    public function muestraAlumnos(){
        ...
        echo"<thead><tr><th>Alumnos:</th></tr></thead><br><br>";

        $sql01 ="SELECT * FROM usuario WHERE status=1 and Nivel=3 ORDER BY apellido_paterno ASC";
        $result01= mysql_query($sql01) or die ("Error $sql01");
        $cuantos = mysql_num_rows($result01);
        for($i = 0; $i < $cuantos; $i++){

            $id_alum=mysql_result($result01,$i,'id_usuario');
            $nombre=mysql_result($result01,$i,utf8_decode('nombre'));
            $app=mysql_result($result01,$i,utf8_decode('apellido_paterno'));
            $apm=mysql_result($result01,$i,utf8_decode('apellido_materno'));
            ...
            if($asignado == ""){
                echo"<tbody><tr><td><input type='checkbox' name='al$i' value='$id_alum'>&nbsp;&nbsp;$nombre &nbsp; $app &nbsp; $apm</td></tr>";
            }
            else{
            ...
            echo"<tr><td>$nombre &nbsp; $app &nbsp; $apm, &nbsp; asignado(a) al grupo $nombregrupo &nbsp; <a href='TestGrupo.php?id=$id_alum'>->Desasignar.</a></td></tr></tbody>";
            }
        }
